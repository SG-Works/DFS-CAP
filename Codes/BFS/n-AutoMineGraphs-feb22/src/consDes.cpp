#include"super.h"
freqEpisodes::freqEpisodes(double expTime1,int tresh1,double entTresh1,int maxLen1,int maxWid1,double fudgeFac1,int maxSiz1,int maxEvents1, char* file,string outfile1,int epi1,bool LmaxFlag1, int NoTimeTicks1, double rho1 )	
{
	expTime = expTime1;
...
freqEpisodes::alpha::alpha(int len)
{
	...
	expiryTime = 0.0;
	pNext = NULL;
	pPrev= NULL; //addition for doubly linked.
	blocStart = 0;
	for(int i=0;i<len;i++)
		for(int j=0;j<len;j++)
