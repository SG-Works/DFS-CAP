...
void freqEpisodes::initCandidates(stEpisodeGel& GelHead)
{
	...
	for(int i=0;i<maxEvents;i++)
	{
		...
		if(ptrPrev != NULL) {ptrPrev->pNext = ptrTemp; ptrTemp->pPrev=ptrPrev; ptrTemp->pstBlockStart = GelHead.ptrAlpha;}//addition st 2.
...
void freqEpisodes::findFreqCandids(stEpisodeGel& GelHead)
{
	...
	for(alpha* alphaIter = GelHead.ptrAlpha;alphaIter!=NULL;alphaIter = alphaNext)
	{
		...
		if( fcuk < entTresh || alphaIter->iFrequency < tresh)
		{
			...
			if(alphaPrev == NULL)
			{
				//earlier version GelHead.ptrAlpha = alphaIter->pNext;
				GelHead.ptrAlpha = alphaNext;
				if(alphaNext != NULL)
					alphaNext->pPrev = NULL; //addition
			}
			else
			{
				alphaPrev->pNext = alphaNext;
				if(alphaNext != NULL)
					alphaNext->pPrev = alphaPrev; //addition.
			}
