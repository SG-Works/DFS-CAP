...
void freqEpisodes::parser(string candid)
{
	...
	//maxSiz is a public variable associated with the object theOne of class freqEpisodes.
	...
	for(int canIter=0;canIter<numCandids;canIter++)
	{
		...
		ptrAlpha->pNext = gelHeads[size-1].ptrAlpha; //introducing the new candidate alpha to the beginning of the list.
	...
	//to do the statistical threshold calculation.
	//bool single = 0; // as we are dealing with multiple event-types per time tick.
	//for(int i=0;i<gelHeads.size();i++)
	//{
		//for(alpha* ptrIter1 = gelHeads[i].ptrAlpha;ptrIter1 != NULL;ptrIter1=ptrIter1->pNext)
		//{
			//int size = i+1; // OR gelHeads[i].iEventLength
			//ptrIter1->FreqThresholdStatistical = ComputeSignificanceThreshold(ptrIter1->paiEvents, ptrIter1->ppEventMatrix, single, 
			//maxEvents, NoTimeTicks, rho, size); 
			//send these pointers and acess as ptrIter1->paiEvents etc.
		//}
	//}
