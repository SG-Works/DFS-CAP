...
class freqEpisodes
{
	int tresh; int maxSiz;int maxEvents;double expTime,fudgeFac;
	char filename[100];
	class alpha;
	class beta
	{
	...
	public:
		...
		alpha* pNext;
		double entropy;		 
		void calculate_entropy(int len);
		void initialize(int);
		void deleteMat(int);
...
public:
	...
	void filter();
	freqEpisodes(double expTime,int tresh,double fugeFac1,int maxSiz,int maxEvents,char*);
};
//CONSTRUCTORS & Destructors -- begin
freqEpisodes::freqEpisodes(double expTime1,int tresh1,double fudgeFac1,int maxSiz1,int maxEvents1, char* file)	
{
	expTime = expTime1;
...
void freqEpisodes::alpha::calculate_entropy(int len)
{
...
void freqEpisodes::findFreqCandids(stEpisodeGel& GelHead)
{
	...
	while((Event = evntStream.getnextevent()) != -1)
	{
		...
		for(beta* betaIter = waitsList[Event -1];betaIter != NULL;betaIter = betaIterNext)
		{
			...
			//wNext[j] = 0;
			bool ADDWAITS = true;
			// finding the index of q_next -- if it exits
	...
	for(alpha* alphaIter = GelHead.ptrAlpha;alphaIter!=NULL;alphaIter = alphaNext)
	{
		...
		{ autoIterNext = autoIter->pNext; autoIter->deleteMat(eLen); delete autoIter;}// leftout
		alphaIter->automataList = NULL;
		alphaIter->calculate_entropy(eLen);
		if(alphaIter->iFrequency < tresh)
		{
	...
	for(alpha* mainIter = prevHead.ptrAlpha;mainIter!=NULL;mainIter = mainIter->pNext)
	{
		...
		for(alpha* blockIter=mainIter->pNext;blockIter!=NULL && blockIter->pstBlockStart == mainIter->pstBlockStart;blockIter=blockIter->pNext)
		{
			...
			for(int iTemp=0;iTemp<3 && posEpisodes[iTemp] != NULL;iTemp++)
			{
				...
				for(int jTemp = 0; (jTemp<l) && (addFlag == true);jTemp++)
				{
							
#if 1
					for(int kTemp=0;kTemp<jTemp-1;kTemp++)
					{
...
#define MAXLEN 100
void freqEpisodes::parser(char *filename, int iEventLength)
{
	FILE *pfCandid = fopen(filename,"r");
...
int main()
{
	double expTime,fudgeFac; int freqTresh,maxEvents,maxSiz;
	...
	freqEpisodes theOne(expTime,freqTresh,fudgeFac,maxSiz,maxEvents,infile);
