...
void freqEpisodes::alpha::display(int len)
{
	for(int i=0;i<len;i++)
		cout << paiEvents[i]<<" ";
	cout << "\t: ";
	cout << "freq " << iFrequency<<"\t: ";
	cout << "BiEv " << entropy<<"\t: ";
	cout << "FreqStatTh " << FreqThresholdStatistical << "\t: ";
	for(int i=0;i<len;i++)
	{
		for(int j=0;j<len;j++)
			if(ppEventMatrix[i][j])
				// cout<<paiEvents[i]<<"->"<<paiEvents[j]<<" "; //injective episodes
				cout<< i+1 <<"->"<< j + 1<<" ";
	}
	cout << endl;
}
...
{	
	...
	for(int i=0;i<len;i++)
	{
		for(int j=0;j<len;j++)
			if(copy[i][j])
				//out<<paiEvents[i]<<">"<<paiEvents[j]<<" "; for injective episodes.
				out<<i+1<<">"<<j+1<<" "; 
	}
	for(i=0;i<len;i++)
		delete []copy[i];
