...
void generateStream(vector<deque<vector<int> > > Q,parameters param)
{
	...
	for(double i=0;i<=param.simTime;i=i+deltaTee)
	{
		for(int j=0;j<Q.size();j++)
		{
			...
			else if(mode[j] == false && myRand(0,1) < param.p) 
			{
		...
		//just to check whether it is generating only noise at a rate of rho when the no. of patterns is zero.
		cout << "No. of Episode Events: " << episodeEvents.size() << endl;
		cout << "No. of Noise Events: " << interRand.size() << endl;
		cout << "No. of patterns as seen by the code " << Q.size() << endl;
#endif
	fout.close();
