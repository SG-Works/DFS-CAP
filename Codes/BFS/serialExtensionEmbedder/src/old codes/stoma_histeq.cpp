...
void generateStream(vector<deque<vector<int> > > Q,double rho,double eta, int numNeu,double simTime)
{
	...
	vector<int> interRand;
	vector<int> intraRand;
	vector<bool> flags;
	...
	for(double i=0;i<=simTime;i=i+deltaTee)
	{
		if(mode == false && myRand(0,1) > rho) 
		{
			...
			vector<bool> flagss;
			for(int i=0;i<numNeu;i++)
				flagss.push_back(false);
			for(int j=0;j<len;j++)
			{
				int eventt = Q[episode][serial][j];
				flagss[eventt-1]=true;
			}
			for(int i=0;i<flagss.size();i++)
				if(flagss[i]==false)
					intraRand.push_back(i+1);
		}
		if(mode == true)
		{
			if(myRand(0,1) > eta)
			...
			{
				int idx = myRandInt(0,intraRand.size()-1);
			        fout<<intraRand[idx]<<","<<i<<endl;
			}
		}
		else
		{
			int idx = myRandInt(0,interRand.size()-1);
			fout<<interRand[idx]<<","<<i<<endl;
		}
